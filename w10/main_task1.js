function main()
{
    ...
    for ( var i = 0; i < 256; i++ )
    {
        ...
        var R = Math.max( Math.cos( ( S - 0.8 ) * Math.PI * (1/0.7) ), 0.0 );
        var G = Math.max( Math.cos( ( S - 0.45 ) * Math.PI * (1/0.7) ), 0.0 );
        var B = Math.max( Math.cos( ( S - 0.1 ) * Math.PI * (1/0.7) ), 0.0 );
    ...
    for ( var i = 0; i < nfaces; i++ )
    {
        ...
        var S1 = scalars[ id[1] ];
        var S2 = scalars[ id[2] ];
        var C0 = new THREE.Color().setHex( cmap[ (int)(((S0-0.1)/0.7)*255) ][1] );
        var C1 = new THREE.Color().setHex( cmap[ (int)(((S1-0.1)/0.7)*255) ][1] );
        var C2 = new THREE.Color().setHex( cmap[ (int)(((S2-0.1)/0.7)*255) ][1] );
